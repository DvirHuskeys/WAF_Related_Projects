# Validation Report
**Document:** docs/sprint-artifacts/1-4-sample-data-loader.context.xml
...
Evidence: AcceptanceCriteria captures sentences such as "Given sample files (`data/samples/domains.csv`, `data/rules/cloudflare_sample.json`) exist, when I run `python scripts/seed_sample_data.py`, then DuckDB `domain_enrichment` includes at least four rows and `managed_rules` includes at least two rows matching the samples.".
